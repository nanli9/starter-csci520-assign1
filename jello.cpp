...
void world::findNeighbors() {
    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 8; j++) {
            for (int k = 0; k < 8; k++) {
                //find structural
                processNeighbors(i, j, k, 1, 0, 0, structuralType);
                processNeighbors(i, j, k, 0, 1, 0, structuralType);
                processNeighbors(i, j, k, 0, 0, 1, structuralType);
                processNeighbors(i, j, k, -1, 0, 0, structuralType);
                processNeighbors(i, j, k, 0, -1, 0, structuralType);
                processNeighbors(i, j, k, 0, 0, -1, structuralType);
                //find shear
                processNeighbors(i, j, k, 1, 1, 0, shearType);
                processNeighbors(i, j, k, -1, 1, 0, shearType);
                processNeighbors(i, j, k, -1, -1, 0, shearType);
                processNeighbors(i, j, k, 1, -1, 0, shearType);
                processNeighbors(i, j, k, 0, 1, 1, shearType);
                processNeighbors(i, j, k, 0, -1, 1, shearType);
                processNeighbors(i, j, k, 0, -1, -1, shearType);
                processNeighbors(i, j, k, 0, 1, -1, shearType);
                processNeighbors(i, j, k, 1, 0, 1, shearType);
                processNeighbors(i, j, k, -1, 0, 1, shearType);
                processNeighbors(i, j, k, -1, 0, -1, shearType);
                processNeighbors(i, j, k, 1, 0, -1, shearType);

                processNeighbors(i, j, k, 1, 1, 1, shearType);
                processNeighbors(i, j, k, -1, 1, 1, shearType);
                processNeighbors(i, j, k, -1, -1, 1, shearType);
                processNeighbors(i, j, k, 1, -1, 1, shearType);
                processNeighbors(i, j, k, 1, 1, -1, shearType);
                ...
                processNeighbors(i, j, k, 1, -1, -1, shearType);
                //find bend
                processNeighbors(i, j, k, 2, 0, 0, bendType);
                processNeighbors(i, j, k, 0, 2, 0, bendType);
                processNeighbors(i, j, k, 0, 0, 2, bendType);
                processNeighbors(i, j, k, -2, 0, 0, bendType);
                processNeighbors(i, j, k, 0, -2, 0, bendType);
                processNeighbors(i, j, k, 0, 0, -2, bendType);
            }
...
void world::processNeighbors(int i, int j, int k, int di, int dj, int dk,SpringType type) {
    int ip = i + (di); 
    int jp = j + (dj);
    int kp = k + (dk);
    if(!((ip > 7) || (ip < 0) || 
