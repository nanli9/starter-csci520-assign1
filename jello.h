...
struct point 
{
   ...
   double y;
   double z;
   //operator overload to make life easier 
...
struct neighbors {
    std::vector<neighbor> structuralNeighborList;
    std::vector<neighbor> shearNeighborList;
    std::vector<neighbor> bendNeighborList;
};
// these variables control what is displayed on the screen
extern int shear, bend, structural, pause, viewingMode, saveScreenToFile, addTexture,toggleSinglePoint,mark, pickedPoint[3];
enum SpringType {
    structuralType = 1,
    shearType = 2,
    bendType = 3
